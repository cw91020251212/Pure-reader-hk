...
首頁有「匯入內容」工作區，「選擇文檔」同「貼上文字」用兩個分頁共用同一介面。大型內容區預設完全收起，按分頁或「大展開」先顯示；可用「收起」按鈕關閉，文檔或貼文完成後亦會自動收合。文檔分頁可按「瀏覽文檔」開啟系統選檔介面，亦可直接拖放文檔；面板會即時顯示已選檔名、大小同解析狀態。支援 `.html`、`.htm`、`.md`、`.markdown`、`.txt`、`.pdf`、`.doc`、`.docx`、`.docm`、`.rtf`、`.odt`、`.epub`、PowerPoint、Excel 同 `.csv`。PDF 會逐頁抽取文字；DOCX 會轉換標題、段落、列表同基本格式；舊式 DOC 及其他辦公格式會按需要載入 WebAssembly 解析器再轉成 Markdown。所有解析都喺瀏覽器本機完成，檔案唔會上傳到伺服器。
