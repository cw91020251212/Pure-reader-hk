...
首頁「上傳檔案」支援 `.html`、`.htm`、`.md`、`.txt`、`.pdf` 同 `.docx`。PDF 會逐頁抽取文字；DOCX 會轉換標題、段落、列表同基本格式。解析喺瀏覽器本機完成，檔案唔會上傳到伺服器。舊式 `.doc` 格式瀏覽器未能可靠解析，請先用 Word 或其他文字處理器另存為 `.docx`。

「貼上文字」及文章完成後的「追加圖片」都支援一次多選或分批連續加入多張圖片。每張圖片會壓縮並內嵌到文章；其 **↑／↓** 控制可把圖片越過標題、段落及其他圖片，移到文章內任意位置。
