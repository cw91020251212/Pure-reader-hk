...
首頁保留單一「貼上文字」按鈕；展開工作頁後可貼入文字、一次加入多張圖片，最下方另有「選擇文檔」按鈕，亦支援直接把文檔拖到該區。選檔後會即時顯示檔名、大小同解析狀態。支援 `.html`、`.htm`、`.md`、`.markdown`、`.txt`、`.pdf`、`.doc`、`.docx`、`.docm`、`.rtf`、`.odt`、`.epub`、PowerPoint、Excel 同 `.csv`。PDF 會逐頁抽取文字；DOCX 會轉換標題、段落、列表同基本格式；舊式 DOC 及其他辦公格式會按需要載入 WebAssembly 解析器再轉成 Markdown。所有解析都喺瀏覽器本機完成，檔案唔會上傳到伺服器。
